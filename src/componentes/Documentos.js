// componentes/Documentos.js
import metadata from "../DocumentosDGA/metadata.json";
...
export default function Documentos(){
  ...
  // Archivos en src/DocumentosDGA
  const archivos = useMemo(() => {
    try {
      const ctx = require.context("../DocumentosDGA", true, /.*/);

      return ctx.keys()
        .filter(key => !key.includes("metadata")) // ignorar metadata.json
        .map((key, idx) => {
          const url = ctx(key);
          ...
          return {
            id: idx + 1,
            nombre,
            descripcion: meta.descripcion || "Documento disponible para descarga.",
      ...
      console.warn("No se pudo cargar src/DocumentosDGA. ¿Existe la carpeta?", e);
      return [];
    }
  }, []);
  ...
  return (
    <Wrapper>
          ...
          <tbody>
            ...
            {archivos.length === 0 && (
              <tr><td colSpan="5" style={{opacity:.7, padding:16}}>
                No hay archivos en <b>src/DocumentosDGA</b>.
              </td></tr>
            )}
          </tbody>
        </Table>
      ...
      <DownloadBar>
        <Btn onClick={onDescargar} disabled={!seleccion}>Descargar documento</Btn>
      </DownloadBar>
    </Wrapper>
  );
}